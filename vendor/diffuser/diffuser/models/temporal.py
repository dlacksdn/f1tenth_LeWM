...
class ValueFunction(nn.Module):
    ...
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            # f1tenth fork-patch: 원본은 is_last 블록도 Downsample1d(L183)를 적용하면서 horizon 변수만
            # is_last에 //2를 건너뜀(off-by-one). standard config(horizon 1로 붕괴+max(,1) 클램프)에선
            # 가려졌으나 f1tenth(H128, dim_mults(1,4,8))서 fc_dim mismatch crash. Downsample1d=
            # Conv1d(k3,s2,p1)→ceil(L/2)라 모든 블록서 (horizon+1)//2로 추적(standard 무회귀 검증).
            horizon = (horizon + 1) // 2
        ...
        horizon = (horizon + 1) // 2   # f1tenth fork-patch: ceil(L/2) 정합
        ##
