...
import numpy as np
import einops
try:  # f1tenth glue (P0): headless, NullRenderer 사용 → imageio 미설치 graceful
    import imageio
except ImportError:
    imageio = None
...
import gym
try:  # f1tenth glue (P0): MuJoCo 미설치 graceful (MuJoCoRenderer는 메서드 내부서만 mjc 참조)
...
try:  # f1tenth glue (P0): video는 imageio 의존
    from .video import save_video, save_videos
except ImportError:
    save_video = save_videos = None

try:  # f1tenth glue (P0): d4rl 미설치 graceful
