...
## 9. B0 실측 결과 (append, 2026-06-20) — γ·RTG 선검증 + reward 조정 비교

> §8.4의 B0를 실제 실행. Track A 학습(GPU) 중 병행한 **무비용 CPU 계산**(npz의 reward로 RTG 직접
> 계산 = value 타깃 sequence.py:146 미러: `RTG[start]=Σ γ^k·r[start+k]`, scipy `lfilter` backward).
> **결론: §8의 "γ vs reward 조정 vs 데이터" 우선순위가 데이터로 확정됨 — γ=0.999가 Track B value의
> 1차 레버이고, reward 조정(완주 보너스↑/충돌 페널티↑)은 전부 무효.**

### 9.1 γ 스윕 RTG (median; 충돌임박은 010 §5의 8.87과 일치 검증)
| γ | 완주c10 | 완주c5 | 고속c20 | 고속c15 | 충돌임박 | 완주>충돌고속 |
|---|---|---|---|---|---|---|
| **0.99**(현재) | 20.1 | 9.7 | **31.4** | 29.3 | 8.9 | ❌ |
| 0.997 | 72.5 | 33.3 | 99.8 | 101.6 | 9.2 | ❌ |
| **0.999** | **217.5** | 102.2 | 151.9 | 187.4 | 9.2 | ✅ |
| 0.9995 | 351.1 | 211.7 | 170.9 | 211.7 | 9.2 | ✅ |

발견:
1. **충돌-임박은 모든 γ서 저평가(8.9~9.2)** — "곧 충돌하는 plan"(윈도에 충돌 포함)은 γ=0.99서도 이미
   완주보다 낮음(충돌 −10이 코앞이라 할인 안 됨). → **009 B1(충돌 벌점 증폭)은 풀려고 한 문제가 이미
   풀려 있어 헛수고.**
2. **진짜 문제 = 속도 편애**: γ0.99서 충돌고속(31) > 안전완주(20). P6에서 guidance가 충돌·고속으로 민
   정량 근거(010 §5 확증).
3. **γ0.999서 역전**: 완주(217) > 충돌고속(152) > 충돌임박(9). 랩 보너스(+200)가 살아나(0.99선 소멸)
   value가 올바로 변별. **cap10 같은 빠른 완주가 최고점 → stretch에 유리.**

### 9.2 reward 조정 vs γ (γ=0.99 고정, 완주 보너스/충돌 페널티 배율 재구성)
`reward' = reward + (lap_w−1)·log_reward_lap + (crash_w−1)·log_reward_collision` (npz 성분 분리저장).
| 시나리오 | 완주c10 | 완주c5 | 고속c20 | 충돌임박 | 완주>충돌 | c10/c5(속도변별) |
|---|---|---|---|---|---|---|
| baseline γ0.99 | 20.1 | 9.7 | 31.4 | 8.9 | ❌ | 2.07 |
| 완주보너스 ×10 | 20.5 | 9.8 | 31.7 | 8.9 | ❌ | 2.09 |
| 완주보너스 ×100 | 25.9 | 9.9 | 32.5 | 8.9 | ❌ | 2.62 |
| 충돌페널티 ×10 | 20.1 | 9.7 | 31.0 | **−38.5** | ❌ | 2.07 |
| **γ0.999** | 217.5 | 102.2 | 151.9 | 9.2 | **✅** | 2.13 |

- **완주 보너스 ↑ = 거의 무효**: ×100조차 완주(25.9)가 충돌고속(32.5)을 못 넘음(❌). 보너스가 랩 라인
  (수백~1400 step 앞)에 있어 γ=0.99로 **소멸**. 효과 보려면 ×10000급 극단 → 스케일 폭발 + 빠른/느린
  완주 변별(c10/c5)까지 뭉갬.
- **충돌 페널티 ↑ = 충돌-임박만 −38.5로 떨구고 나머지 불변**(고속 31.4→31.0, 완주 20.1 불변, ❌). 속도
  편애 못 고침.
- ★ **핵심 직관**: 문제는 완주 보너스의 *크기*가 아니라 *거리*(멀어서 할인으로 죽음). **reward 조정은
  크기를, γ는 거리를 건드린다.** 멀리 있는 표지판 — 글씨를 키우기(보너스↑)보다 다가가기(γ↑)가 정답이고,
  글씨 키우면 가까운 글씨(progress=속도)와 균형까지 깨진다. reward를 "완주를 dense하게" 재설계해도 결국
  progress 강화라 속도 편애 악화 → γ가 가장 깔끔.

### 9.3 결론 — Track B value 레버 확정
- ★ **value 재학습 = γ=0.999 단독.** 009 B1(벌점 증폭)·reward 조정(완주 보너스↑) **전부 폐기**(무효 실증).
- **§8.1(γ) 확증.** **§8.2(고품질 데이터)는 "stretch 필수"→"2차 보강"으로 완화** — γ만으로 guidance
  방향 교정 가능, 데이터는 "더 빠른 완주 재료"로 stretch를 *더* 밀 때.
- **010 §4("value는 완주를 못 본다")는 γ=0.99 한정** 결론이었음 — γ를 키우면 완주가 변별됨(실증).
- **트레이드오프/유보**: γ0.999면 RTG 스케일 217~351 → value 학습 난이도↑(`normed=True`가 [-1,1] 압축,
  P4 corr 0.98 유지 재확인 필요). "데이터에 변별 신호 존재"는 ✅이나, value가 학습으로 근사 + closed-loop
  에서 실제로 미는지는 **Track B 실험**으로만 확정. 안전(완주)은 여전히 prior(Track A) 책임.
- 재현: `/home/dlacksdn/f1tenth_RL_project/.venv/bin/python`로 RTG = lfilter backward(γ 스윕 + lap_w/crash_w
  배율), 데이터 `runs/crash_data`(동결). (휘발성 스크립트, 본 표가 결과.)
